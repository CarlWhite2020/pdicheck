...
class ConnectionImpl extends Connection {
    ...
    ConnectionImpl(BTManager btManager,
    ...
    private boolean hasConnectPermission(Context context) {
        ...
        if (Build.VERSION.SDK_INT >= 31) {
            return PermissionChecker.hasPermission(context,
    ...
    @Override
    public void connect(final UUID uuid, final ConnectCallback callback) {
        if (isReleased) {
            if (callback != null) {
    ...
    synchronized void changeState(int state,
                                  boolean noEvent) {
        ...
        this.state = state;
        BTLogger.INSTANCE.d(BTManager.DEBUG_TAG,
                "Connection state changed: " + getStateDesc(state));
        if (!noEvent) {
            callback(
                    MethodInfoGenerator.onConnectionStateChanged(device, state)
            );
        }
    }
    ...
    @Override
    public void write(@Nullable String tag,
                      @NonNull byte[] value,
    ...
    @Override
    public void writeImmediately(@Nullable String tag,
                                 @NonNull byte[] value,
                                 @Nullable WriteCallback callback) {
    ...
    private void write(String tag,
        ...
        if (isReleased || !bluetoothAdapter.isEnabled()) {
            if (callback != null) {
                ...
                this.callback(
                        MethodInfoGenerator.onWrite(device,
                                tag,
                                value,
                                false)
                );
