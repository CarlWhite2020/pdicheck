...
public class BTManager {
    private static final int RSSI = -120;
            ...
            if (action != null) {
                ...
                switch (action) {
                    ...
                    case BluetoothAdapter.ACTION_DISCOVERY_STARTED:
                        isDiscovering = true;
                        handleDiscoveryCallback(true, null, RSSI, -1, "");
                        break;
                    case BluetoothAdapter.ACTION_DISCOVERY_FINISHED:
                        isDiscovering = false;
                        handleDiscoveryCallback(false, null, RSSI, -1, "");
                        break;
    ...
    public void startDiscovery() {
        ...
        synchronized (this) {
            ...
            if (!isLocationEnabled(application)) {
                ...
                handleDiscoveryCallback(false, null, RSSI, DiscoveryListener.ERROR_LOCATION_SERVICE_CLOSED, errorMsg);
            ...
            if (noLocationPermission(application)) {
                ...
                handleDiscoveryCallback(false, null, RSSI, DiscoveryListener.ERROR_LACK_LOCATION_PERMISSION, errorMsg);
                BTLogger.INSTANCE.e(DEBUG_TAG, errorMsg);
                return;
            }
            if (noScanPermission(application)) {
                String errorMsg = "Unable to scan for Bluetooth devices, lack scan permission.";
                handleDiscoveryCallback(false, null, RSSI, DiscoveryListener.ERROR_LACK_SCAN_PERMISSION, errorMsg);
                BTLogger.INSTANCE.e(DEBUG_TAG, errorMsg);
                return;
            }
            //部分机型获取蓝牙名称需要连接权限，无语
            if (noConnectPermission(application)) {
                ...
                handleDiscoveryCallback(false, null, RSSI, DiscoveryListener.ERROR_LACK_CONNECT_PERMISSION, errorMsg);
                BTLogger.INSTANCE.e(DEBUG_TAG, errorMsg);
                return;
            }
        }
    ...
    public Connection createConnection(@NonNull BluetoothDevice device, EventObserver observer) {
        if (checkStatus()) {
            ...
            if (connectionMap.containsKey(device.getAddress())) {
                return connectionMap.get(device.getAddress());
            }
     ...
     */
    @SuppressLint("MissingPermission")
    public boolean createBond(@NonNull final String address) {
        checkStatus();
        try {
            BluetoothDevice remoteDevice =
                    bluetoothAdapter.getRemoteDevice(address);
            return remoteDevice.getBondState() != BluetoothDevice.BOND_NONE
     ...
     */
    @SuppressLint("MissingPermission")
    public boolean createBond(@NonNull final BluetoothDevice device) {
        checkStatus();
        try {
